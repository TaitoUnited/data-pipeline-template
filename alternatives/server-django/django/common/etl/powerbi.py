...
def as_powerbi_table_schema(table_name, database, table_name_prefix="public "):
    ...
    for name in df:
        columns.append(
            {"name": name, "dataType": as_powerbi_datatype(df[name].dtype)}
        )
    return {"name": table_name_prefix + table_name, "columns": columns}


def as_powerbi_table_data(
    table_name, database, order_by="id", offset=0, limit=None
):
    validate_is_one_word(table_name)
    rows = []
...
class PowerBIDataset:
    ...
    def copy_table_data(
        self, table_name, order_by, database, delete_data=False, limit=10000
    ):
        validate_is_one_word(table_name)
        ...
        while True:
            data = as_powerbi_table_data(
                table_name, database, order_by, offset=i * limit, limit=limit
            )
            if len(data["rows"]) <= 0:
                return
