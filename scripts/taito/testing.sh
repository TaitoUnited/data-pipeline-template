...
if [[ $taito_target_env == "local" ]]; then
  ...
  ci_test_base_url=http://data-pipeline-template-ingress:80
else
  ...
  if [[ $taito_command == "util-test" ]]; then
    ...
    ci_test_base_url=https://$(cat tmp/secrets/dev/data-pipeline-template-dev-basic-auth.auth | sed 's/:{PLAIN}/:/')@$taito_domain
  fi
fi
